...
const MyAssignments = () => {
  ...
  const loadAssignments = () => {
    ...
    fetch('/api/assignments/my-assignments', { credentials: 'include' })
      ...
      .catch((err) => {
        setLoadError(err.message || 'Failed to load your assignments.');
      })
  ...
  const handleRevisionSubmit = async (e) => {
    e.preventDefault();
    if (!activeRevisionModal) return;
    setRevisionError('');
    setRevisionSubmitting(true);
    try {
      const body = new URLSearchParams();
      body.append('reason', revisionReason);

      const res = await fetch(`/assignments/${activeRevisionModal.id}/request-revision`, {
        method: 'POST',
        credentials: 'include',
      ...
      // Spring's redirect-based handler returns an opaque redirect on success
      if (res.type === 'opaqueredirect' || res.ok) {
        setActiveRevisionModal(null);
        setRevisionReason('');
        loadAssignments(); // refresh with real data
        return;
      }

      const text = await res.text().catch(() => '');
      throw new Error(text || 'Failed to submit revision request.');
    } catch (err) {
      ...
      <nav className="bg-[#212529] text-white py-3 px-6 shadow-md sticky top-0 z-50">
        <div className="container mx-auto flex flex-wrap justify-between items-center gap-4">
          ...
          <div className="flex items-center gap-4 ml-auto">
            {user.fullName && (
              <span className="font-medium hidden sm:flex items-center gap-2 text-white/80">
                <i className="bi bi-person-circle text-xl"></i> {user.fullName}
              </span>
            )}
      ...
      <div className="container mx-auto px-4 py-8 flex-1 max-w-5xl">
        ...
        {loading && (
          <div className="text-center py-20 bg-white rounded-2xl shadow-sm border border-gray-100">
            <i className="bi bi-arrow-repeat animate-spin text-4xl text-[#667eea] mb-4 block"></i>
        ...
        {!loading && loadError && (
          <div className="text-center py-16 bg-white rounded-2xl shadow-sm border border-red-100">
            <i className="bi bi-exclamation-triangle text-4xl text-red-400 mb-4 block"></i>
            <p className="text-red-600 font-medium mb-4">{loadError}</p>
            <button onClick={loadAssignments} className="bg-[#3498db] text-white font-bold px-6 py-2.5 rounded-xl hover:bg-[#2980b9] transition-colors">
                  ...
                  <div className="p-6 md:p-8">
                    <div className="flex flex-col md:flex-row gap-6">
                      ...
                      <div className="flex-1">
                        ...
                        {/* Revision Info */}
                        {(assignment.status === 'DELIVERED' || assignment.status === 'REVISION_REQUESTED') && assignment.maxRevisions != null && (
                      ...
                      <div className="w-full md:w-48 flex flex-col gap-3 justify-start border-t md:border-t-0 md:border-l border-gray-100 pt-6 md:pt-0 md:pl-6">
                        ...
                        {assignment.status === 'DELIVERED' && assignment.maxRevisions != null && assignment.revisionsUsed < assignment.maxRevisions && (
                          <button
          ...
          <div className="bg-white rounded-[15px] w-full max-w-lg overflow-hidden shadow-2xl animate-scaleUp">
            ...
            <form onSubmit={handleRevisionSubmit} className="p-6">
              ...
              <div className="flex justify-end gap-3">
                ...
                <button type="submit" disabled={revisionSubmitting} className="px-6 py-2.5 rounded-xl bg-gradient-to-r from-[#f093fb] to-[#f5576c] text-white font-bold shadow-md hover:shadow-lg transition-all flex items-center gap-2 disabled:opacity-50">
                  <i className="bi bi-send"></i> {revisionSubmitting ? 'Submitting…' : 'Submit Request'}
                </button>
              </div>
            </form>
          </div>
