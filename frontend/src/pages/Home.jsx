...
const Home = () => {
  ...
  const prevSlide = () => {
    ...
    setCurrentSlide(p => (p === 0 ? testimonials.length - 1 : p - 1));
  };
  ...
  const navItems = ['hero','services','how-it-works','testimonials','contact'];
  const navLabels = ['Home','Services','How It Works','Testimonials','Contact'];

  return (
      <div style={{ fontFamily: "'Segoe UI', Tahoma, Geneva, Verdana, sans-serif", color: '#333', lineHeight: 1.6, overflowX: 'hidden' }}>

        {/* ── NAVBAR ── */}
        <nav style={{
          position: 'fixed', top: 0, width: '100%', zIndex: 1000,
          padding: '10px 0', transition: 'all 0.5s ease',
          backgroundColor: isScrolled || mobileMenuOpen ? '#212529' : 'transparent',
          boxShadow: isScrolled ? '0 5px 15px rgba(0,0,0,0.1)' : 'none',
        }}>
          <div className="navbar-inner" style={{ maxWidth: 1200, margin: '0 auto', padding: '0 20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            {/* Desktop links */}
            <div className="navbar-links-desktop" style={{ display: 'flex', alignItems: 'center', gap: 20, flexWrap: 'wrap' }}>
              {navItems.map((id, i) => (
            ...
            <button
              ...
              style={{ display: 'none', background: 'none', border: 'none', color: '#fff', fontSize: 26, cursor: 'pointer', padding: 4 }}
            >
          ...
          {mobileMenuOpen && (
            <div className="navbar-mobile-menu" style={{ background: '#212529', padding: '10px 20px 20px', display: 'flex', flexDirection: 'column', gap: 4 }}>
              {navItems.map((id, i) => (
                  <button key={id} onClick={() => scrollToSection(id)} style={{ background: 'none', border: 'none', color: '#fff', fontWeight: 500, cursor: 'pointer', fontSize: 16, padding: '12px 8px', textAlign: 'left', borderBottom: '1px solid rgba(255,255,255,0.08)' }}>
                    {navLabels[i]}
                  </button>
              ))}
              <button onClick={() => scrollToSection('pricing')} style={{ background: 'none', border: 'none', color: '#fff', fontWeight: 500, cursor: 'pointer', fontSize: 16, padding: '12px 8px', textAlign: 'left', borderBottom: '1px solid rgba(255,255,255,0.08)' }}>Pricing</button>
              <div style={{ display: 'flex', gap: 12, marginTop: 12 }}>
                <Link to="/login" onClick={() => setMobileMenuOpen(false)} style={{ flex: 1, textAlign: 'center', background: '#fff', color: '#212529', padding: '10px 20px', borderRadius: 20, fontWeight: 600, textDecoration: 'none' }}>Login</Link>
                <Link to="/register" onClick={() => setMobileMenuOpen(false)} style={{ flex: 1, textAlign: 'center', background: '#667eea', color: '#fff', padding: '10px 20px', borderRadius: 20, fontWeight: 600, textDecoration: 'none' }}>Register</Link>
              </div>
            </div>
          )}
        </nav>
        ...
        <section id="hero" style={{
          ...
          <div style={{ maxWidth: 800, padding: '0 20px', width: '100%' }}>
            <h1 style={{ fontSize: 'clamp(30px,7vw,60px)', fontWeight: 600, marginBottom: 20 }}>
              Professional Assignment Help for IT &amp; QS Students
            </h1>
            <p style={{ maxWidth: 700, margin: '10px auto 30px', fontSize: 'clamp(14px,2.2vw,18px)', opacity: 0.95 }}>
        ...
        <div style={{ textAlign: 'center', margin: '70px 0 30px', fontFamily: "'Segoe UI', Tahoma, Geneva, Verdana, sans-serif" }}>
          ...
          <h2 style={{ margin: 0, fontSize: 'clamp(26px,5vw,36px)', fontWeight: 800, color: '#212529' }}>What We Offer</h2>
        </div>
        <section className="programs-strip" style={{ margin: '0 auto 40px', width: '90%', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 20, flexWrap: 'wrap' }}>
          {[
            ...
            { img: 'https://images.unsplash.com/photo-1516321318423-f06f85e504b3?auto=format&fit=crop&w=800', icon: 'https://img.icons8.com/ios-filled/50/ffffff/online-support.png', label: '24/7 Support' },
              ...
              <div key={i} className="programs-strip-item" style={{ flex: '1 1 30%', position: 'relative', overflow: 'hidden', borderRadius: 10, minWidth: 200 }}
        ...
        <section id="services" style={{ padding: '100px 0', background: '#fff', position: 'relative', overflow: 'hidden' }}>
          <div style={{ maxWidth: 1200, margin: '0 auto', padding: '0 24px', position: 'relative', zIndex: 2 }}>
            <div style={{ textAlign: 'center', marginBottom: 60 }}>
              ...
              <h2 style={{ fontSize: 'clamp(28px,5vw,40px)', fontWeight: 800, color: '#212529', marginBottom: 14 }}>Our Specialized Services</h2>
            ...
            {/* Stats Bar */}
            <div className="stats-bar" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'linear-gradient(135deg,#0f0c29,#302b63)', borderRadius: 20, padding: '32px 40px', flexWrap: 'wrap', gap: 20 }}>
              {[['500+','Projects Completed'],['98%','Satisfaction Rate'],['24/7','Support Available'],['100%','On-time Delivery']].map(([num, txt], i, arr) => (
                  <React.Fragment key={i}>
                    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6, flex: '1 1 40%', minWidth: 120 }}>
                      <span style={{ fontSize: 'clamp(26px,5vw,34px)', fontWeight: 800, background: 'linear-gradient(135deg,#667eea,#a78bfa)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>{num}</span>
                      <span style={{ fontSize: 13, color: 'rgba(255,255,255,0.6)', fontWeight: 500, textAlign: 'center' }}>{txt}</span>
                    </div>
                    {i < arr.length - 1 && <div className="stats-divider" style={{ width: 1, height: 50, background: 'rgba(255,255,255,0.1)', flexShrink: 0 }}></div>}
                  </React.Fragment>
              ))}
            </div>
          </div>
        ...
        <section id="pricing" style={{ padding: '100px 0', background: '#fff' }}>
          <div style={{ maxWidth: 1200, margin: '0 auto', padding: '0 24px' }}>
            <div style={{ textAlign: 'center', marginBottom: 60 }}>
              ...
              <h2 style={{ fontSize: 'clamp(28px,5vw,40px)', fontWeight: 800, color: '#212529', marginBottom: 12 }}>Popular Packages</h2>
        ...
        <section id="testimonials" style={{ padding: '100px 0', background: 'linear-gradient(135deg,#0f0c29 0%,#302b63 50%,#24243e 100%)', position: 'relative', overflow: 'hidden' }}>
          ...
          <div style={{ maxWidth: 1200, margin: '0 auto', padding: '0 24px' }}>
            <div style={{ textAlign: 'center', marginBottom: 60, position: 'relative', zIndex: 2 }}>
              ...
              <h2 style={{ fontSize: 'clamp(28px,5vw,40px)', fontWeight: 800, color: '#fff', marginBottom: 12 }}>What Our Clients Say</h2>
        ...
        <section id="contact" style={{ padding: '100px 0', background: '#f8f9fa', position: 'relative', overflow: 'hidden' }}>
          <div style={{ maxWidth: 1200, margin: '0 auto', padding: '0 24px' }}>
            <div style={{ textAlign: 'center', marginBottom: 60 }}>
              ...
              <h2 style={{ fontSize: 'clamp(28px,5vw,40px)', fontWeight: 800, color: '#212529', marginBottom: 12 }}>Contact Us</h2>
                  ...
                  { icon: 'bi-clock-fill', bg: 'linear-gradient(135deg,#fa709a,#fee140)', label: 'Support Hours', value: '24/7 Customer Support' },
                    ...
                    <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 16, padding: 14, background: 'rgba(255,255,255,0.1)', borderRadius: 14, marginBottom: 14, border: '1px solid rgba(255,255,255,0.15)', transition: 'all 0.3s', textDecoration: 'none', color: '#fff' }}>
                      ...
                      <div style={{ minWidth: 0 }}>
                        ...
                        <span style={{ fontSize: 14, color: '#fff', fontWeight: 500, wordBreak: 'break-word' }}>{item.value}</span>
                      </div>
                    </div>
                ))}
                      ...
                      <li key={i} style={{ marginBottom: 12 }}>
                        <a href={item.href || '#'} target={item.href?.startsWith('http') ? '_blank' : undefined} rel="noreferrer" style={{ display: 'flex', alignItems: 'center', gap: 14, padding: 12, background: 'rgba(255,255,255,0.05)', borderRadius: 12, border: '1px solid rgba(255,255,255,0.1)', textDecoration: 'none', color: '#fff', transition: 'all 0.3s' }}
                          ...
                          <div style={{ minWidth: 0 }}>
                            ...
                            <span style={{ fontSize: 13, fontWeight: 500, wordBreak: 'break-word' }}>{item.value}</span>
                          </div>
                        </a>
                      </li>
                  ))}
        ...
        <style>{`
          ...
          @keyframes testimonialSlideInRight { from{opacity:0;transform:translateX(60px)} to{opacity:1;transform:translateX(0)} }
          @keyframes testimonialSlideInLeft { from{opacity:0;transform:translateX(-60px)} to{opacity:1;transform:translateX(0)} }
...
const ServiceCard = ({ iconBg, icon, tag, tagColor, tagBg, borderTop, title, desc, items, checkBg, checkColor, btnBg, btnShadow, btnLabel }) => {
  ...
  return (
      <div onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}
           style={{ background: '#fff', borderRadius: 24, padding: '40px 28px', border: hovered ? '1.5px solid transparent' : '1.5px solid #f0f0f0', position: 'relative', overflow: 'hidden', transition: 'all 0.4s', transform: hovered ? 'translateY(-10px)' : 'none', boxShadow: hovered ? '0 24px 60px rgba(0,0,0,0.1)' : '0 4px 24px rgba(0,0,0,0.05)' }}>
        ...
        <h3 style={{ fontSize: 22, fontWeight: 800, color: '#212529', marginBottom: 12 }}>{title}</h3>
        <p style={{ fontSize: 14.5, color: '#6c757d', lineHeight: 1.7, marginBottom: 24 }}>{desc}</p>
        <ul style={{ listStyle: 'none', padding: 0, margin: '0 0 30px', borderTop: '1px solid #f0f0f0', paddingTop: 20, display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(140px,1fr))', gap: 10 }}>
          {items.map((item, i) => (
...
const PricingCard = ({ icon, name, desc, price, currency, period, perPerson, featured, badge, plus, features }) => {
  ...
  return (
      <div onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}
           style={{ position: 'relative', borderRadius: 24, padding: featured ? '48px 28px' : '40px 28px', border: featured ? 'none' : '1.5px solid #f0f0f0', background: featured ? 'linear-gradient(135deg,#0f0c29,#302b63)' : '#fff', color: featured ? '#fff' : '#212529', boxShadow: hovered ? '0 20px 50px rgba(0,0,0,0.15)' : '0 4px 20px rgba(0,0,0,0.05)', transform: hovered ? 'translateY(-8px)' : 'none', transition: 'all 0.3s' }}>
        ...
        <div style={{ marginBottom: 4 }}>
          ...
          <span style={{ fontSize: 38, fontWeight: 800, margin: '0 4px' }}>{price}</span>
