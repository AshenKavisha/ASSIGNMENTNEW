  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      if (!userResponse.ok) {
        ...
        navigate('/login?error=Invalid email or password');
        return;
      }

      const user = await userResponse.json();

      // Step 3 — Make sure the logged in user matches what we tried to login with
      // This prevents old sessions from bypassing login
      if (user.email.toLowerCase() !== email.toLowerCase()) {
        navigate('/login?error=Invalid email or password');
        return;
      }

      // Step 4 — Redirect based on real role from backend
      if (user.role === 'ADMIN') {
      ...
      navigate('/login?error=Something went wrong. Please try again.');
    } finally {
      setIsLoading(false);
    }
  };
